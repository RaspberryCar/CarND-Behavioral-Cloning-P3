...
class TPlot:
  ...
  def get_time_image(self, image_path):
    ...
    return datetime.datetime(year = int(time[1]), month = int(time[2]), day = int(time[3]),
                             hour = int(time[4]), minute = int(time[5]), second=int(time[6]), microsecond=int(time[7].split(".")[0]))


  def _fetch_from_csv(self):
    self._df_simu = pd.read_csv(self._csv_path, usecols=[0,1,2,3], names=['cfile', 'lfile', 'rfile', 'angle_normalize'])
  ...
  def prepare_plot_simu(self,fig, ax):
    ...
    for i in range(N):
        ...
        html = '<img src="'+ src +'" width="'+ str(self._image_width/2) +'" height="'+ str(self._image_height/2) +'">'
        html += '<table>'
        ...
        html += '</table>'
        labels.append(html)
    points = ax.plot(self._df_simu.ctime, self._df_simu.angle_normalize, 'o', color='b', mec='k', ms=15, mew=1, alpha=.6)
  ...
  def prepare_plot_real(self,fig, ax):
    ...
    for i in range(N):
        ...
        html += '</table>'
        labels.append(html)
    points = ax.plot(self._df_real.time, self._df_real.angle_normalize, 'o', color='b', mec='k', ms=15, mew=1, alpha=.6)
